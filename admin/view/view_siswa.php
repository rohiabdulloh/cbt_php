...
create_button("info", "print", "Cetak Label", "btn-print", "form_print_label()");

//Membuat header dan footer tabel
create_table(array("NIS", "Nama Siswa", "Password", "Kelas", "Ruang", "Aksi"));
...
open_form("modal_siswa", "return save_data()");
   ...
   create_textbox("Ruang", "ruang", "text", 4, "", "required"); 
close_form();
...
//Membuat form cetak label
open_form("modal_label", "return print_label()");
   $qkelas = mysqli_query($mysqli, "SELECT * FROM kelas");
   $list = array();
   while($rk = mysqli_fetch_array($qkelas)){
      $list[] = array($rk['id_kelas'], $rk['kelas']);
   }
   create_combobox("Kelas", "kelas_label", $list, 4, "", "required");
close_form("print", "Print");
