   ...
   while($r = mysqli_fetch_array($query)){
      ...
      if($r['jenis'] == 0){
         ...
         for($i=1; $i<=5; $i++){	
            $kolom = "pilihan_$i";
            if (!empty($r[$kolom])) {             
               $ind = isset($kunciArray[$i - 1]) ? (int) $kunciArray[$i - 1] : -1;
               $paramText = ($ind >= 0 && isset($parameter[$ind-1])) ? $parameter[$ind-1] : '';

               $soal .= '<li>' . $r[$kolom] . 
               ($paramText !== '' ? ' <span class="text-primary fw-bold">(' . $paramText . ')</span>' : '') . 
               '</li>';
            }
         }
         $soal .= '</ul>';
