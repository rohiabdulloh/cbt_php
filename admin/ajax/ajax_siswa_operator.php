...
if($_GET['action'] == "table_data"){
   ...
   while($r = mysqli_fetch_array($query)){
	  ...
	  $row[] = '<a class="btn btn-default active" onclick="Lock_login('.$r['nis'].')"><i class="glyphicon glyphicon-off"></i> Lock APP</a>';
      $data[] = $row;
      $no++;
   }
...
elseif($_GET['action'] == "UNLock_login"){
   mysqli_query($mysqli, "UPDATE siswa set status='off' , jmlog = 0 ,browser='close app' WHERE status<>'0'");
}
