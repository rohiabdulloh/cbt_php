...
if($_GET['action'] == "table_data"){
   $query = mysqli_query($mysqli, "SELECT * FROM siswa ORDER BY id_kelas, nama");
   $data = array();
   $no = 1;
...
elseif($_GET['action'] == "import"){
   ...
   if($extensi != ".xls"){
      ...
      for($i=2; $i<=$jdata; $i++){		
		 ...
		 $ruang	= addslashes($data->val($i,4));
				
         $cek = mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM siswa WHERE nis='$nis'"));
         if($cek > 0){
            mysqli_query($mysqli, "UPDATE siswa SET nama='$nama',    no_ruang='$ruang ',id_kelas='$_POST[kelas_import]' WHERE  nis='$nis'");
         }else{
            $pass = md5(substr(md5($nis),0,5));
            mysqli_query($mysqli, "INSERT INTO siswa SET nis='$nis', nama='$nama',  no_ruang='$ruang',  id_kelas='$_POST[kelas_import]', password='$pass', status='off'");
         }
      }	
