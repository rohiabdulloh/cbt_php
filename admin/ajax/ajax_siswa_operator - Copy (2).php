...
if($_GET['action'] == "table_data"){
   ...
   while($r = mysqli_fetch_array($query)){
      ...
      elseif(strtolower($r['status'] == "mengerjakan")) $status = '<b class="text-danger">mengerjakan</b>';
	   elseif(strtolower($r['status'] == "lock")) $status = '<b class="text-danger">lock</b>';
	   elseif($r['jmlog'] >= 3 ) $status = '<b class="text-danger">lock</b>';
      else $status = '<b class="text-muted">logout</b>';
...
elseif($_GET['action'] == "reset_login"){
   mysqli_query($mysqli, "UPDATE siswa set status='logout' , jmlog = 0  WHERE nis='$_GET[nis]'");
}
...
elseif($_GET['action'] == "UNLock_login"){
   mysqli_query($mysqli, "UPDATE siswa set status='logout' , jmlog = 0 ,browser='close app' , ip =' ' WHERE status<>'0'");
}
