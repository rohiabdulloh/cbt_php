   ...
   $.ajax({
      ...
      success : function(data){
         if(data=="ok"){
            ...
            showError("Terjadi permasalahan pada server");
            $('#nis').focus();
         }
      },
      error : function(){
...
//Ketika tombol Cetak Label diklik
function form_print_label(){
   $('#modal_label').modal('show');
   $('.modal-title').text('Cetak Label Bangku');
   $('#modal_label form')[0].reset();
}

//Ketika tombol Cetak pada modal label diklik
function print_label(){
   $('#modal_label').modal('hide');
window.open("export/pdf_label.php?kelas="+$('#kelas_label').val(), "Cetak Label Bangku", "height=650, width=1024, left=150, scrollbars=yes");
   return false;
}

//Ketika tombol export diklik
function form_export() {
  window.location.href = "export/format_siswa.xls";
}
